...
class SendAuthCode_API(object):
    @Base_API("sql_script_request.ini","sql_script_post_loginModule","sendAuthCode")
    def sendAuthCode_api(obj):
        body = obj[1]
        assert Assertions().assert_body(body,'msg','请求成功')
    """
    def __call__(self, *args, **kwargs):
        source = Post_request_sqlData()
        source("sql_script_request.ini","sql_script_post_loginModule","sendAuthCode")
